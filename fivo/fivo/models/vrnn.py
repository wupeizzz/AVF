...
class VRNN(object):
  ...
  def __init__(self,
    ...
    self.forwardmcmc = forwardmcmc
    self.backwardmcmc = backwardmcmc
  ...
  def forward_transition(self, auxiliary_latent):
      return self.forwardmcmc(auxiliary_latent)

  def backward_transition(self, latent):
      return self.backwardmcmc(latent)
...
class TrainableVRNN(VRNN, base.ELBOTrainableSequenceModel):
  ...
  def __init__(self,
    ...
    super(TrainableVRNN, self).__init__(
        ...
        transition, emission, forwardmcmc, backwardmcmc, random_seed=random_seed)
  ...
  def propose_and_weight(self, state, t):
    ...
    # start MCMC transition
    q_zt1 = self.forwardmcmc(zt)
    zt1 = q_zt1.sample(seed=self.random_seed)
    b_zt0 = self.backwardmcmc(zt1)
    p_xt1_given_zt1, latent_encoded = self.emission(zt1, rnn_out)
    log_p_xt1_given_zt1 = tf.reduce_sum(p_xt1_given_zt1.log_prob(targets), axis=-1)
    log_b_zt0 =  tf.reduce_sum(b_zt0.log_prob(zt), axis=-1)
    log_p_zt1 = tf.reduce_sum(p_zt.log_prob(zt1), axis=-1)
    log_q_zt1 = tf.reduce_sum(q_zt1.log_prob(zt1), axis=-1)
    alpha_weight = log_p_xt1_given_zt1 + log_p_zt1 + log_b_zt0 -\
                   (log_p_zt + log_p_xt_given_zt + log_q_zt1)

    total_weight = weights + alpha_weight
...
def create_vrnn(
  ...
  if auxiliary_latent_size is None:
    auxiliary_latent_size =  latent_size
  ...
  transition = base.ConditionalNormalDistribution(
      ...
      initializers=initializers,
      name="prior")
  forwardmcmc = base.ConditionalNormalDistribution(
      size=auxiliary_latent_size,
      hidden_layer_sizes=fcnet_hidden_sizes,
      initializers=initializers,
      hidden_activation_fn = tf.nn.tanh,
      name="forwardmcmc") # q
  backwardmcmc = base.ConditionalNormalDistribution(
      size=latent_size,
      hidden_layer_sizes=fcnet_hidden_sizes,
      initializers=initializers,
      hidden_activation_fn=tf.nn.tanh,
  ...
  return TrainableVRNN(
      ...
      emission, forwardmcmc, backwardmcmc, proposal_type, proposal=proposal, rev_rnn_cell=rev_rnn_cell,
